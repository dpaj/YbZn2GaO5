...
# =============================================================================
#
# This variant parameterizes neutron intensity with a formal overall scale and a single gperp_ratio.\n# This standalone file embeds the two original source scripts in private modules
...
const YZGO_FIT_WINDOWS_1D = Dict(
    "0_1_0"        => [(1.1, 3.0)],
    "0p33_0p33_0" => [(0.6, 3.0)],#[(0.75, 1.6), (2.3, 3.0)],
    "0p5_0_0"     => [(0.6, 3.0)],#[(0.75, 1.6), (2.3, 3.0)],
)
...
# =============================================================================
# USER-EDITABLE CO-FIT SETTINGS
# =============================================================================
# Keep the numbers that a co-worker is most likely to change in one visible
# place.  The functions below still accept keyword arguments, but the defaults
# are now drawn from these constants instead of being scattered through the
# driver.

const LAST_COFIT_BEST_INITIAL_GUESSES = (;
    # Shared Hamiltonian / disorder parameters.
    gzz = 3.784584444,
    J1_meV = 0.2299824987,
    J2_meV = 0.008722296371,
    sigma_gzz = 0.3859237343,
    sigma_J = 0.2396873058,   # shared fractional sigma for J1 and J2

    # Non-dispersive S=1/2 component.
    gzz2 = 2.998557026,
    sigma_gzz2 = 0.8439361137,

    # Neutron-only transverse matrix-element ratio.
    # Fitted neutron-only transverse intensity ratio.  This is NOT an absolute
    # physical g_perp.  Because an overall neutron scale is also fit, the
    # dispersive transverse matrix-element scale is fixed by convention and only
    # the flat/dispersive ratio is identifiable.
    gperp_ratio = 2.967716763,

    # Magnetization-only extrinsic/phenomenological slope.
    chi_vv_muB_per_T = 0.07826757513,

    # Shared non-dispersive fraction / relative kernel intensity.
    log10_second_kernel_relative_intensity = -0.8013917578,
    second_kernel_relative_intensity = 0.1579822309,

    # Neutron absolute scale from the latest shared-sigmaJ co-fit.
    log10_neutron_scale = -1.348641118,
    neutron_global_scale = 0.04480834274,
)

# Internal gauge used only when calling the older neutron helper routines that
# still expect keys named :gperp and :gperp2.  Do not interpret this as a fitted
# or physical Yb transverse g factor; it fixes the arbitrary absolute transverse
# neutron intensity scale, while :gperp_ratio carries the fitted relative value.
const NEUTRON_TRANSVERSE_MATRIX_ELEMENT_GAUGE = 1.0

const DEFAULT_COFIT_CONTROLS = (;
    # Data selection.
    fields_T = (9.0, 14.0),
    ...
    # Objective weights.  These are extrinsic choices, not Hamiltonian values.
    neutron_weight = 1.0,
    magnetization_weight = 10.0,

    # Optimizer / Monte-Carlo controls.
    maxiters = 1000,
    ...
    # Magnetization comparison window and uncertainty model.
    Bmin_fit_T = 0.0,
    Bmax_fit_T = 7.0,
    magnetization_sigma_muB = nothing,
    magnetization_error_fraction_of_range = 0.02,
    magnetization_error_floor_muB = 1e-4,
...
function print_cofit_control_summary(; base_dir, magnetization_csv, outdir, fields_T,
                                      neutron_fit_Ei_meV, neutron_fit_temperature_K,
                                      qtags, data_mode, neutron_weight,
                                      magnetization_weight, maxiters,
                                      n_samples_per_cut, final_n_samples_per_cut,
                                      n_samples_magnetization,
                                      final_n_samples_magnetization, seed, mag_seed,
                                      Bmin_fit_T, Bmax_fit_T, run_optimization)
    println("Co-fit user-editable controls")
    println("-----------------------------")
    println("base_dir                 = ", base_dir)
    println("magnetization_csv        = ", magnetization_csv)
    println("outdir                   = ", outdir)
    println("neutron fields_T         = ", collect(fields_T))
    println("neutron Ei/T filter      = ", neutron_fit_Ei_meV, " meV, ", neutron_fit_temperature_K, " K")
    println("qtags                    = ", collect(qtags))
    ...
    println(@sprintf("magnetization fit range  = %.8g to %.8g T", Float64(Bmin_fit_T), Float64(Bmax_fit_T)))
    println("run_optimization         = ", run_optimization)
    println("maxiters                 = ", maxiters)
    println("MC samples neutron       = ", n_samples_per_cut, " opt, ", final_n_samples_per_cut, " final")
    println("MC samples magnetization = ", n_samples_magnetization, " opt, ", final_n_samples_magnetization, " final")
    println("random seeds             = neutron ", seed, ", magnetization ", mag_seed)
    println()
end
...
function cofit_default_param_specs(; initial_shared_r2::Real=LAST_COFIT_BEST_INITIAL_GUESSES.second_kernel_relative_intensity,
                                    initial_gzz::Real=LAST_COFIT_BEST_INITIAL_GUESSES.gzz,
                                    initial_J1_meV::Real=LAST_COFIT_BEST_INITIAL_GUESSES.J1_meV,
                                    initial_J2_meV::Real=LAST_COFIT_BEST_INITIAL_GUESSES.J2_meV,
                                    initial_sigma_gzz::Real=LAST_COFIT_BEST_INITIAL_GUESSES.sigma_gzz,
                                    initial_sigma_J::Real=LAST_COFIT_BEST_INITIAL_GUESSES.sigma_J,
                                    initial_gzz2::Real=LAST_COFIT_BEST_INITIAL_GUESSES.gzz2,
                                    initial_sigma_gzz2::Real=LAST_COFIT_BEST_INITIAL_GUESSES.sigma_gzz2,
                                    initial_gperp_ratio::Real=LAST_COFIT_BEST_INITIAL_GUESSES.gperp_ratio,
                                    initial_chi_vv_muB_per_T::Real=LAST_COFIT_BEST_INITIAL_GUESSES.chi_vv_muB_per_T,
                                    initial_neutron_global_scale::Real=LAST_COFIT_BEST_INITIAL_GUESSES.neutron_global_scale,
                                    gzz_bounds=(1.0, 8.0),
                                    J1_bounds=(0.0, 0.60),
                                    J2_bounds=(0.0, 0.40),
                                    sigma_gzz_bounds=(0.0, 1.5),
                                    sigma_J_bounds=(0.0, 2.0),
                                    gzz2_bounds=(0.5, 8.0),
                                    sigma_gzz2_bounds=(0.0, 2.0),
                                    gperp_ratio_bounds=(0.0, 8.0),
                                    chi_vv_bounds=(0.0, 0.30),
                                    log10_shared_r2_bounds=(-4.0, 4.0),
                                    log10_neutron_scale_bounds=(-12.0, 12.0))
    # Important: only the ratio of transverse neutron matrix elements is identifiable
    # once an overall neutron scale is included as a fit parameter. We therefore use
    # gperp_ratio = gperp_flat / gperp_disp and absorb gperp_disp^2 into
    # neutron_global_scale = 10^log10_neutron_scale.
    return NF.FitParamSpec[
        ...
        NF.FitParamSpec(:sigma_J,   Float64(sigma_J_bounds[1]),   Float64(sigma_J_bounds[2]),   Float64(initial_sigma_J)),
        ...
        NF.FitParamSpec(:gperp_ratio,
                        Float64(gperp_ratio_bounds[1]), Float64(gperp_ratio_bounds[2]),
                        Float64(initial_gperp_ratio)),
        ...
        NF.FitParamSpec(:log10_neutron_scale,
                        Float64(log10_neutron_scale_bounds[1]),
                        Float64(log10_neutron_scale_bounds[2]),
                        log10(Float64(initial_neutron_global_scale))),
    ]
end
function neutron_param_dict_from_cofit(p::Dict{Symbol,Float64})
    pn = Dict{Symbol,Float64}()
    for nm in [:gzz, :J1_meV, :J2_meV, :sigma_gzz, :gzz2, :sigma_gzz2]
        pn[nm] = p[nm]
    end
    # Shared fractional exchange disorder: use the same normalized width for J1 and J2.
    pn[:sigma_J1] = p[:sigma_J]
    pn[:sigma_J2] = p[:sigma_J]

    if haskey(p, :gperp_ratio)
        # Gauge choice for neutron intensity:
        #   I = scale_neutron * (I_disp + r2 * gperp_ratio^2 * I_flat)
        # The absolute dispersive transverse matrix-element scale is absorbed
        # into scale_neutron.  The legacy key :gperp is therefore set to an
        # internal convention value, not a physical or fitted parameter.
        pn[:gperp] = NEUTRON_TRANSVERSE_MATRIX_ELEMENT_GAUGE
        pn[:gperp2] = NEUTRON_TRANSVERSE_MATRIX_ELEMENT_GAUGE * max(p[:gperp_ratio], 0.0)
    else
        # Backwards-compatible path for older specs with separate gperp/gperp2.
        if haskey(p, :gperp)
            pn[:gperp] = p[:gperp]
        end
        if haskey(p, :gperp2)
...
neutron_global_scale_from_cofit(p::Dict{Symbol,Float64}) =
    haskey(p, :log10_neutron_scale) ? 10.0 ^ p[:log10_neutron_scale] : NaN
...
# -----------------------------------------------------------------------------
# Magnetization model with shared fractional exchange disorder sigma_J
# -----------------------------------------------------------------------------
...
function draw_local_env_Bsat_and_msat_cofit(p::Dict{Symbol,Float64}, draws;
    ...
    sJ1_abs = p[:sigma_J] * abs(p[:J1_meV])
    sJ2_abs = p[:sigma_J] * abs(p[:J2_meV])
    Sval = Float64(S)
...
function write_cofit_summary(path::AbstractString;
    ...
    open(path, "w") do io
        ...
        println(io, "  Neutron-only relative transverse prefactor: gperp_ratio = flat/dispersive matrix-element ratio")
        println(io, "  Neutron:       I = scale_neutron * (I_disp + r2 * gperp_ratio^2 * I_nondispersive)")
        println(io, "  Note: legacy :gperp is fixed to a gauge value only; it is not fitted or physical.")
        ...
        for nm in [:gzz, :J1_meV, :J2_meV, :sigma_gzz, :sigma_J,
                   ...
                   :log10_second_kernel_relative_intensity, :log10_neutron_scale]
            if haskey(params, nm)
                println(io, @sprintf("  %-56s % .12g", String(nm), params[nm]))
            end
        end
        println(io, @sprintf("  %-56s % .12g", "r2_shared", shared_r2_from_cofit(params)))
        if haskey(params, :gperp_ratio)
            println(io, @sprintf("  %-56s % .12g", "gperp_ratio_squared", params[:gperp_ratio]^2))
        end
...
function run_yzgo_neutron_magnetization_cofit_shared_fraction(; 
        ...
        fields_T::AbstractVector{<:Real}=Float64[DEFAULT_COFIT_CONTROLS.fields_T...],
        neutron_fit_Ei_meV::Union{Nothing,Real}=DEFAULT_COFIT_CONTROLS.neutron_fit_Ei_meV,
        neutron_fit_temperature_K::Union{Nothing,Real}=DEFAULT_COFIT_CONTROLS.neutron_fit_temperature_K,
        qtags=String[DEFAULT_COFIT_CONTROLS.qtags...],
        fit_windows_by_q=NF.YZGO_FIT_WINDOWS_1D,
        data_mode::Symbol=DEFAULT_COFIT_CONTROLS.data_mode,
        neutron_weight::Real=DEFAULT_COFIT_CONTROLS.neutron_weight,
        magnetization_weight::Real=DEFAULT_COFIT_CONTROLS.magnetization_weight,
        specs::Union{Nothing,Vector{NF.FitParamSpec}}=nothing,
        maxiters::Int=DEFAULT_COFIT_CONTROLS.maxiters,
        n_samples_per_cut::Int=DEFAULT_COFIT_CONTROLS.n_samples_per_cut,
        final_n_samples_per_cut::Int=DEFAULT_COFIT_CONTROLS.final_n_samples_per_cut,
        seed::Int=DEFAULT_COFIT_CONTROLS.seed,
        n_samples_magnetization::Int=DEFAULT_COFIT_CONTROLS.n_samples_magnetization,
        final_n_samples_magnetization::Int=DEFAULT_COFIT_CONTROLS.final_n_samples_magnetization,
        mag_seed::Int=DEFAULT_COFIT_CONTROLS.mag_seed,
        quad_n::Int=DEFAULT_COFIT_CONTROLS.quad_n,
        Bmin_fit_T::Real=DEFAULT_COFIT_CONTROLS.Bmin_fit_T,
        Bmax_fit_T::Real=DEFAULT_COFIT_CONTROLS.Bmax_fit_T,
        Bmin_plot_T::Real=0.0,
        Bmax_plot_T::Real=7.0,
        dB_plot_T::Real=0.01,
        magnetization_sigma_muB::Union{Nothing,Real}=DEFAULT_COFIT_CONTROLS.magnetization_sigma_muB,
        magnetization_error_fraction_of_range::Real=DEFAULT_COFIT_CONTROLS.magnetization_error_fraction_of_range,
        magnetization_error_floor_muB::Real=DEFAULT_COFIT_CONTROLS.magnetization_error_floor_muB,
        use_errors::Bool=true,
        error_floor::Real=0.0,
        show_trace::Bool=true,
        trace_every::Int=5,
        make_plots::Bool=DEFAULT_COFIT_CONTROLS.make_plots,
        display_figures::Bool=DEFAULT_COFIT_CONTROLS.display_figures,
        run_optimization::Bool=DEFAULT_COFIT_CONTROLS.run_optimization,
    ...
    print_cofit_control_summary(;
        base_dir=base_dir,
        magnetization_csv=magnetization_csv,
        outdir=outdir,
        fields_T=fields_T,
        neutron_fit_Ei_meV=neutron_fit_Ei_meV,
        neutron_fit_temperature_K=neutron_fit_temperature_K,
        qtags=qtags,
        data_mode=data_mode,
        ...
        final_n_samples_magnetization=final_n_samples_magnetization,
        seed=seed,
        mag_seed=mag_seed,
        Bmin_fit_T=Bmin_fit_T,
        Bmax_fit_T=Bmax_fit_T,
    ...
    obj = function(u)
        ...
        if trace_every > 0 && (eval_counter[] == 1 || eval_counter[] % trace_every == 0)
            pnow = metrics.p
            if !isempty(pnow)
                @printf("eval %4d  obj=%.8g  nχ²=%.8g  mχ²=%.8g  gzz=%.5g J1=%.5g J2=%.5g sigg=%.5g sigJ=%.5g gzz2=%.5g sigg2=%.5g gperp_ratio=%.5g r2=%.5g nscale=%.5g\n",
                    eval_counter[], metrics.total, metrics.neutron.redchi2, metrics.magnetization.redchi2,
                    pnow[:gzz], pnow[:J1_meV], pnow[:J2_meV], pnow[:sigma_gzz], pnow[:sigma_J],
                    pnow[:gzz2], pnow[:sigma_gzz2], get(pnow, :gperp_ratio, NaN), shared_r2_from_cofit(pnow),
                    haskey(pnow, :log10_neutron_scale) ? 10.0^pnow[:log10_neutron_scale] : metrics.neutron.scale)
            else
    ...
    if haskey(pbest, :log10_neutron_scale)
        println(@sprintf("  %-56s % .10g", "neutron_global_scale", neutron_global_scale_from_cofit(pbest)))
        println(@sprintf("  %-56s % .10g", "log10_neutron_scale", pbest[:log10_neutron_scale]))
        println(@sprintf("  %-56s % .10g", "gperp_ratio_squared", pbest[:gperp_ratio]^2))
        println(@sprintf("  %-56s % .10g", "effective_flat_weight_r2_times_ratio2", r2_shared_final * pbest[:gperp_ratio]^2))
    end
...
# Backward-compatible alias for this standalone shared-fraction script.
run_yzgo_neutron_magnetization_cofit(; kwargs...) =
    run_yzgo_neutron_magnetization_cofit_shared_fraction(; kwargs...)
